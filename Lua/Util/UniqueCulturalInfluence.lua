...
function ChangeInfluentialText()
    for iPlayer = 0, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
        ...
        if player ~= nil and player:IsAlive() and player:IsHuman() then
			...
			--print ( locale )
			for _ in DB.Query("SELECT Text FROM " ..locale .." WHERE Tag='" ..tag .."'") do
				text = _.Text
			end
			if text then
				--print (text)
				for _ in DB.Query("UPDATE " ..locale .." SET Text = '" ..text .."' WHERE Tag='TXT_KEY_GENERIC_INFLUENTIAL_ON_AI_1'") do
				end
